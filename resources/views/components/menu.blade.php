    ...
    <div class="container mx-auto px-4 max-w-7xl">
        ...
        <div class="mb-8 bg-[#FDF8F3] rounded-lg p-6 shadow-sm">
            <form method="GET" action="{{ route('menu.index') }}"
                ...
                <button type="submit"
                    class="w-full sm:w-auto px-6 py-3 bg-[#E2CEB1] text-white rounded-lg hover:bg-[#D4BFA2] transition-all duration-300 font-medium text-lg mt-4 sm:mt-8">
                    Show Menu
                </button>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            @else
                @foreach ($menus as $menu)
                    <div class="group bg-white rounded-xl border-2 border-[#E2CEB1] p-4 hover:shadow-xl transition-all duration-300"
    ...
    function addToCart(menuId, menuName, menuPrice, menuImages) {
        ...
        fetch('{{ route('cart.add') }}', {
            ...
            .then(data => {
                // Success feedback
                button.innerHTML =
                    '<svg class="h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>';
                setTimeout(() => {
