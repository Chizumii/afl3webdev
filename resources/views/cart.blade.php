        ...
        <div id="cart-items" class="space-y-2">
            @if (session('cart') && count(session('cart')) > 0)
                @foreach (session('cart') as $item)
                    <div class="bg-[#f6f6f6]  p-3 transition-all"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="w-16 text-center bg-transparent text-[#A07658] border border-[#E2CEB1] text-sm p-1">
                                    {{ $item['quantity'] }}
                                </span>
                            </div>
    ...
    <footer class="fixed bottom-0 left-0 right-0 bg-white p-4 border-t border-[#ebc892]">
        <div class="container mx-auto">
            <div class="relative flex items-center justify-between">
                <!-- Items count - Left aligned -->
                <div class="text-lg text-[#ebc892] font-normal">
                    Items in Cart: <span
                        id="total-items">{{ session('cart') ? collect(session('cart'))->sum('quantity') : 0 }}</span>
                </div>

                <!-- Confirm Payment Button - Center aligned -->
                ...
                <div class="text-lg text-[#42403f] font-normal">
                    Total: <span
...
<script>
    function updateCartDisplay(cart) {
        ...
        const confirmPaymentButton = document.querySelector('button[onclick="confirmPayment()"]');
        if (confirmPaymentButton) {
            confirmPaymentButton.disabled = totalItems === 0;
        }
    }
