...
<body>
    ...
    <div class="max-w-lg mx-auto p-6 bg-white shadow-md rounded-lg">
        <h2 class="text-2xl font-bold mb-6">Create New Menu</h2>
        <form action="{{ route('menuAdmin.store') }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-lg">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4" 
                    class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="resto_id" id="resto_id" required 
                    ...
                    @foreach ($restaurants as $restaurant)
                        <option value="{{ $restaurant->id }}">{{ $restaurant->resto_name }}</option>
                    @endforeach
                </select>
            </div>
